...
export default function SellDyno() {
  ...
  const sellTokens = async () => {
    ...
    try {
      ...
      const user_to_pool_allowance = await dynoTokenContract.allowance(
        userAddress,
        dynoPoolAddress
      );
      if (user_to_pool_allowance.lt(parsedAmount)) {
        const user_to_pool_approveTx = await dynoTokenContract.approve(
          dynoPoolAddress,
          parsedAmount
        );
      ...
      const pool_to_user_allowance = await mockUsdcContract.allowance(
        dynoPoolAddress,
        userAddress
      );
      if (pool_to_user_allowance.lt(parsedAmount)) {
        const pool_to_user_approveTx = await mockUsdcContract.approve(
          userAddress,
          parsedAmount
        );
      ...
      const pool_to_buyer_allowance = await mockUsdcContract.allowance(
        dynoPoolAddress,
        dynoBuyerAddress
      );
      if (pool_to_buyer_allowance.lt(parsedAmount)) {
        const pool_to_buyer_approveTx = await mockUsdcContract.approve(
          dynoBuyerAddress,
          parsedAmount
        );
      ...
      // Sell Dyno Tokens
      const tx = await buyerContract.sellDynoTokens(parsedAmount, {
  ...
  return (
    <div>
      <h2>Sell Dyno Tokens</h2>
      <input
        type="number"
        value={amount}
        onChange={handleInputChange}
        placeholder="Enter amount in DYNO"
        min="0"
        step="any"
        disabled={isLoading}
      />
      <button onClick={sellTokens} disabled={isLoading || !amount}>
